    ...
    for (let i = 0; i < listingEls.length; i++) {
      try {
        ...
        const details = await page.evaluate(() => {
          ...
          let fullAddress = addressNode ? addressNode.textContent.trim().replace(/^[^\w\d]+/, '') : '';
        ...
        addresses.push(details.fullAddress);
        websites.push(details.website);
        imageUrls.push(details.imageUrl);
