...
const OUTPUT_FILE = path.join(OUTPUT_DIR, `${DATE}-${CATEGORY}-${LOCATION}-[step-1]-all.csv`);
...
(async () => {
  ...
  try {
    ...
    await page.evaluate(async () => {
      ...
      const maxScrollDuration = 120000; // 2 minutes to load all
      ...
      while (Date.now() - startTime < maxScrollDuration) {
        ...
        if (stableScrolls >= 10) break; // 10 times stable => no more loading
      }
    });
    ...
    console.log(`🔍 Extracting all ${listingEls.length} listings...`);
    ...
    const businesses = await page.evaluate((CATEGORY, LOCATION, detailsList) => {
      const results = [];
      const listings = Array.from(document.querySelectorAll('div.Nv2PK'));
      let index = 0;
