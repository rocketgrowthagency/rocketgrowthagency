  ...
  try {
    ...
    for (let i = 0; i < listingEls.length; i++) {
      try {
        ...
        const details = await page.evaluate(() => {
          ...
          const businessHoursNode = document.querySelector('[aria-label*="Open"]');
          const businessHours = businessHoursNode ? businessHoursNode.textContent.trim().replace(/^[^\w\d(]+/, '') : '';

          const tagNodes = Array.from(document.querySelectorAll('[aria-label*="Identifies as"], [aria-label*="friendly"]'));
          const ownershipTags = tagNodes.map(el => el.textContent.trim()).join(', ');

          return { fullAddress, website, imageUrl, mapsUrl, latitude, longitude, categoryUI, businessHours, ownershipTags };
        });

        detailsList.push(details);
      } catch {
        detailsList.push({ fullAddress: '', website: '', imageUrl: '', mapsUrl: '', latitude: '', longitude: '', categoryUI: '', businessHours: '', ownershipTags: '' });
      }
    }
    ...
    const businesses = await page.evaluate((CATEGORY, LOCATION, detailsList) => {
      ...
      listings.forEach(listing => {
        ...
        let { fullAddress, website, imageUrl, mapsUrl, latitude, longitude, categoryUI, businessHours, ownershipTags } = detailsList[index];
    ...
    const csvWriter = createCsvWriter({
      ...
      header: [
        ...
        { id: 'ownershipTags', title: 'Ownership Tags' }
      ]
    });
