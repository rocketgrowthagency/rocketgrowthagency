    ...
    const body = {
      ...
      messages: [{
        ...
        content: [
          ...
          { type: 'text', text: 'One frame from a LOCAL-business review video. It should show Google Maps zoomed to the business\'s CITY, WITH a business detail panel/card open (business name + star rating + address), and that card should show real photo thumbnails. Judge: (1) is_map: does the frame show a Google Maps map? (2) zoom: "city" = streets/neighborhood/one town fills the view; "wide" = a whole state/multiple counties/country/continent/large oceans fill it. "na" if not a map. (3) pin: is a red/colored location map-pin visible? (4) card_open: is a business detail/info CARD or PANEL open, showing a business name + rating + address (NOT just a plain list of results with nothing selected)? (5) photos_visible: look at the photo area at the TOP of the open card. Answer TRUE if a real photo/image is shown, OR if it shows Google\'s "no photos available" placeholder (a solid BLUE illustrated building/city-skyline graphic — a legitimate state for a business with no photos). Answer FALSE ONLY if that area is a stark BLANK WHITE or plain GRAY empty box where a real photo clearly failed to load (a loading bug), or there is no card at all. Reply ONLY compact JSON: {"is_map":bool,"zoom":"city"|"wide"|"na","pin":bool,"card_open":bool,"photos_visible":bool}.' },
          { type: 'image_url', image_url: { url: `data:image/jpeg;base64,${b64}` } },
        ],
      }],
