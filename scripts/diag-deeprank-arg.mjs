...
// --- lead from argv/env (variant of diag-deeprank) ---
const LEAD = {
  name: process.env.LEAD_NAME || process.argv[2],
  city: process.env.LEAD_CITY || process.argv[3] || '',
  searchTerm: process.env.LEAD_TERM || process.argv[4] || 'Med spas in Culver City, CA',
  rank: parseInt(process.env.LEAD_RANK || process.argv[5] || '99', 10),
  mapsUrl: '',
};
...
const OUT = path.join(process.cwd(), 'output', 'diag', 'deeprank-' + (LEAD.name||'x').toLowerCase().replace(/[^a-z0-9]+/g,'-').slice(0,30));
