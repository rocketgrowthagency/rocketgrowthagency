...
// Classify a domain's MX: 'no-mx' (no records), 'proxy-mx' (all resolvable MX hosts are CDN proxies —
// can't do SMTP), 'ok' (has a real mail host, or unresolvable → fail open), or null (DNS error → fail open).
async function classifyMx(domain) {
  let mx;
  try { mx = await dns.resolveMx(domain); }
  catch (e) { return (e.code === 'ENOTFOUND' || e.code === 'ENODATA') ? 'no-mx' : null; } // null = transient DNS → fail open
  if (!mx || !mx.length) return 'no-mx';
  let anyReal = false, anyProxy = false;
  for (const rec of mx) {
    const host = String(rec.exchange || '').toLowerCase().replace(/\.$/, '');
    if (!host) continue;
    if (MAIL_HOST_ALLOW.test(host)) { anyReal = true; continue; }
    let ips = [];
    ...
    if (ips.every(isProxyIp)) anyProxy = true;  // every IP for this MX host is a CDN proxy
    else anyReal = true;                        // at least one real (non-proxy) mail IP
  }
  if (anyReal) return 'ok';                     // a real mail host exists → not our problem
  if (anyProxy) return 'proxy-mx';              // MX resolves ONLY to CDN proxies → no mailbox possible
...
module.exports = { verifyEmailLayered, policy, classifyMx, hasMx, isProxyIp, v4InCidr, bouncerUnknownIsUnreachable };
