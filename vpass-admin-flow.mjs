...
console.log('BEFORE:',await p.evaluate(()=>({label:document.querySelector('#pendingActionsBanner [data-pending-go]')?.textContent?.trim(),heading:document.querySelector('#pendingActionsBanner p')?.textContent})));
// Click "Go to client" (the real flow)
await p.evaluate(()=>document.querySelector('#pendingActionsBanner [data-pending-go]')?.click());
await new Promise(z=>setTimeout(z,5000));
console.log('AFTER (inside client):',await p.evaluate(()=>({label:document.querySelector('#pendingActionsBanner [data-pending-go]')?.textContent?.trim(),heading:document.querySelector('#pendingActionsBanner p')?.textContent})));
