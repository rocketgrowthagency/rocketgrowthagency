...
function HomePage() {
  ...
  const handleAuditSubmit = async (event) => {
    ...
    try {
      ...
      // GA4 custom event for Growth Audit submissions
      if (typeof window !== 'undefined') {
        try {
          window.dataLayer = window.dataLayer || [];
          window.dataLayer.push({
            event: 'growth_audit_submit',
            form_name: 'growth_audit',
            form_location: `${window.location.pathname || ''}${window.location.hash || ''}`,
          });
        } catch (err) {
          console.error('GA event push failed', err);
        }
      }
